...
if [ "${EL9_CHECK}" ];then
    yum install gnutls gnutls-devel -y
fi

if [ "${UBUNTU_22_C}" ] || [ "${UBUNTU_24}" ];then
...
Mem_Check(){
    ...
    if [ "${actionType}" == "update" ]; then
        ...
        elif [ "${version}" == "8.0" ] || [ "${version}" == "8.4" ] || [ "${version}" == "9.0" ]; then
            MIN_MEM=4096
...
Download_Src(){
    ...
    if [ "${version}" == "8.0" ] && [ "${actionType}" == "update" ];then
        ...
        if [ -z "${AAPANEL_CHECK}" ];then
            ...
            echo "======================================================================="
            echo "The current MySQL 8.0 version does not support upgrade operations."
            echo "If you need to upgrade to the latest version 8.0.36, you must back up your data and reinstall MySQL 8.0 (compilation method) to use it."
            echo "Note: After the backup, the data may not be compatible with the latest version of MySQL. Please update with caution."
            echo "No changes were made, and the current MySQL operation is not affected."
            exit 0
        fi
    fi
